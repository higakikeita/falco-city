 ...
 *
 * The ids this catalogue covers, against GAME-DESIGN §4 ①:
 *
 *   Web サービス   nginx / nodejs
 *   金融決済       postgres / java
 *   ゲーム基盤     redis / gameserver
 *   製造業         legacy-middleware / old-kernel
 *
 * TIME
 * ----
...
const MIDDLEWARE = [
  ...
  { id:'legacy-middleware', jp:'レガシーミドルウェア', jpKind:'ベンダ製アプライアンス',
    updatable:false,
...
/* The archetype lane is writing its own file right now, so the id it picks may
   be spelled differently. Accept the obvious spellings rather than lose the
   whole catalogue to a hyphen. */
const MW_ALIASES = {
...
/* ---------------------------------------------------------------- catalogue
   `window` is the disclosure DISTRIBUTION: the vulnerability becomes public
...
const VULNS = [
  ...
  { id:'nginx-smuggling', mw:'nginx', code:'FC-2026-0101', sev:'high', inUse:true,
    window:[1,4], jp:'リクエストスマグリングで背後のアプリに素の要求を通す',
  ...
  { id:'nginx-path-traversal', mw:'nginx', code:'FC-2026-0114', sev:'high', inUse:true,
    window:[3,7], jp:'パス正規化の穴で公開範囲外のファイルを読み出す',
  ...
  { id:'nginx-module-rce', mw:'nginx', code:'FC-2026-0132', sev:'crit', inUse:true,
    window:[8,12], jp:'サードパーティモジュールの境界外書き込みから任意コード実行',
  ...
  { id:'node-log-lib', mw:'nodejs', code:'FC-2026-0241', sev:'crit', inUse:false,
    window:[12,18], jp:'ログ整形ライブラリの ReDoS（該当バージョンは未使用）',
    detect:{ via:'exec', newRule:false, rule:'Terminal shell in container', maturity:'stable' },
  ...
  { id:'node-worker-escape', mw:'nodejs', code:'FC-2026-0258', sev:'crit', inUse:true,
    window:[16,22], jp:'ワーカースレッドの分離を破ってホスト名前空間へ出る',
  ...
  { id:'legacy-cred-store', mw:'legacy-middleware', code:'FC-2026-0744', sev:'high', inUse:true,
    window:[10,17], jp:'資格情報を平文で置いているファイルを読み出される',
    detect:{ via:'shadow', newRule:false, rule:'Read sensitive file untrusted', maturity:'stable' },
  ...
  { id:'legacy-vendor-tunnel', mw:'legacy-middleware', code:'FC-2026-0758', sev:'crit', inUse:true,
    window:[14,20], jp:'ベンダ保守トンネルを踏んで内側から外へ出る',
    detect:{ via:'imds', newRule:true,
             rule:'Contact EC2 Instance Metadata Service From Container', maturity:'incubating' },
